...
import datetime, threading, time, math, json, os
...
bot_state = {
    ...
    "trades": [],  # Abgeschlossene Trades mit P&L
    "open_trade": None,  # Aktuell offener Trade
    "stats": {
...
def calculate_all_indicators(prices):
    ...
    rsi = calc_rsi(p)
    macd, macd_s, macd_h = calc_macd(p)
    bb_l, bb_m, bb_u = calc_bollinger(p)
...
def evaluate_signal(inds):
    ...
    sk=inds.get("stoch_k"); adx=inds.get("adx")
    wr=inds.get("williams_r"); cci=inds.get("cci")
    vwap=inds.get("vwap"); mom=inds.get("momentum")
    ...
    if e200:
        if price > e200: bull.append(f"Preis({price}) über EMA200({e200}) — Aufwärtstrend")
        else: bear.append(f"Preis({price}) unter EMA200({e200}) — Abwärtstrend")
    if rsi is not None:
        if rsi < 35: bull.append(f"RSI={rsi} — Überverkauft (Kaufsignal)")
        elif rsi > 65: bear.append(f"RSI={rsi} — Überkauft (Verkaufssignal)")
        elif 40 < rsi < 60: bull.append(f"RSI={rsi} — Neutral bullish")
    if macd and ms:
        if macd > ms: bull.append(f"MACD={macd} > Signal={ms} — Bullish Crossover")
        else: bear.append(f"MACD={macd} < Signal={ms} — Bearish Crossover")
    if bbl and bbu:
        if price < bbl: bull.append(f"Preis unter BB-Unterkante({bbl}) — Oversold")
        elif price > bbu: bear.append(f"Preis über BB-Oberkante({bbu}) — Overbought")
    if sk is not None:
    ...
    if bp >= 60 and len(bull) >= 5: return "BUY", round(bp,1), bull, bear
    elif sp >= 60 and len(bear) >= 5: return "SELL", round(sp,1), bear, bull
    conf = round(max(bp,sp),1)
    return "WARTEN", conf, bull, bear
...
def check_trade(price):
    ...
    if result:
        ...
        update_stats(result, pnl)
        add_log(f"Trade geschlossen: {result} | P&L: {pnl:+.2f} Punkte", "TRADE")
            ...
            if price:
                ...
                if len(bot_state["prices"]) >= 30:
                    ...
                    entry = {
                        ...
                        "reasons": reasons, "counter_reasons": counter,
                        "atr": atr,
                        ...
                        "indicator_count": len(reasons) + len(counter),
                    }
                    ...
                    add_log(f"{sig} | Konfidenz:{conf}% | Preis:{price} | Indikatoren:{len(reasons)+len(counter)}", lvl)
                else:
...
.hdr{background:#111827;border:1px solid #1e3a5f;border-radius:8px;padding:12px 16px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center}
.logo{font-size:18px;font-weight:700;color:#f59e0b;letter-spacing:2px}
...
.pos{color:#4ade80}.neg{color:#f87171}.neu{color:#94a3b8}.amr{color:#f59e0b}
.sm{text-align:center}.sv{font-size:20px;font-weight:700}.sl{font-size:8px;color:#6b7280;letter-spacing:1px;margin-top:2px}
...
.sbox{padding:10px;border-radius:6px;border:1px solid;margin-bottom:6px}
...
<body>
<div class="hdr">
  <div><div class="logo">⚡ XAUUSD KI-BOT DASHBOARD</div><div class="sub">GOLD/USD · AUTONOME ANALYSE · v2.0</div></div>
  <div class="badges">
...
<div class="g4">
  ...
  <div class="pn"><div class="pt"><span class="dot dg pulse"></span>SIGNAL</div>
    <div id="sig-box" class="sbox swait">
      <div style="font-size:14px;font-weight:700;letter-spacing:2px" id="sig-t">WARTEN</div>
      <div style="font-size:10px;color:#94a3b8;margin-top:4px" id="sig-c">Sammle Daten...</div>
    </div>
  </div>
...
<div class="g2">
  <div class="pn">
    ...
    <div class="ir"><span class="in">BB Oben</span><span id="bb-u" class="neu">—</span></div>
    <div class="ir"><span class="in">BB Mitte</span><span id="bb-m" class="neu">—</span></div>
    <div class="ir"><span class="in">BB Unten</span><span id="bb-l" class="neu">—</span></div>
    <div class="ir"><span class="in">Stochastic K</span><span id="stoch" class="neu">—</span></div>
    <div class="ir"><span class="in">Williams %R</span><span id="wr" class="neu">—</span></div>
  ...
  <div class="pn">
    ...
    <div style="font-size:9px;color:#6b7280;margin-bottom:4px">BULLISH SIGNALE</div>
    <div id="bull-reasons" style="font-size:9px;color:#4ade80;line-height:1.8;min-height:60px">Warte auf Daten...</div>
    <div style="font-size:9px;color:#6b7280;margin:8px 0 4px">BEARISH SIGNALE</div>
    <div id="bear-reasons" style="font-size:9px;color:#f87171;line-height:1.8;min-height:60px">Warte auf Daten...</div>
...
<div class="g2">
  <div class="pn">
    ...
    <table>
      <thead><tr><th>DATUM</th><th>DIR</th><th>ENTRY</th><th>CLOSE</th><th>P&L</th><th>RESULT</th></tr></thead>
      <tbody id="trades-body"><tr><td colspan="6" style="color:#6b7280;text-align:center">Noch keine Trades</td></tr></tbody>
    </table>
    </div>
    <div style="margin-top:8px;border-top:1px solid #1e2d45;padding-top:6px;display:grid;grid-template-columns:repeat(3,1fr);gap:6px">
      <div class="sm"><div class="sv pos" id="avg-win">—</div><div class="sl">AVG WIN</div></div>
      <div class="sm"><div class="sv neg" id="avg-loss">—</div><div class="sl">AVG LOSS</div></div>
      <div class="sm"><div class="sv amr" id="signal-count">0</div><div class="sl">SIGNALE</div></div>
    </div>
  </div>
  <div class="pn">
...
<script>
const BASE = "";
function col(v,pos,neg){return v>0?'pos':v<0?'neg':'neu'}
function fmt(v){if(v===null||v===undefined)return'—';return v}
...
async function refresh(){
  try{
    ...
    document.getElementById('sig-t').textContent = st;
    ...
    document.getElementById('avg-loss').textContent = s.avg_loss?s.avg_loss:'—';
    ...
    set('wr',i.williams_r,i.williams_r<-80?'pos':i.williams_r>-20?'neg':'neu');
    ...
    if(ot){
      const cur = d.price||0;
      const upnl = ot.direction==='BUY'?cur-ot.entry:ot.entry-cur;
    ...
    // Trades Table
    const trades = await fetch(BASE+'/trades').then(r=>r.json());
    const tbody = document.getElementById('trades-body');
    if(trades.length){
      tbody.innerHTML = trades.slice(0,20).map(t=>
        `<tr>
          <td>${t.date||''} ${t.close_time||''}</td>
    ...
    document.getElementById('log-box').innerHTML = logs.map(l=>
      `<div class="le" style="color:${l.level==='SIGNAL'?'#f59e0b':l.level==='TRADE'?'#4ade80':l.level==='ERROR'?'#f87171':l.level==='WARN'?'#f59e0b':'#60a5fa'}">
        <span style="color:#374151">${l.time}</span> [${l.level}] ${l.msg}
      </div>`).join('');
...
def webhook():
    try:
        ...
        add_log(f"TradingView: {data}", "INFO")
        if data.get("price"):
            try:
...
if __name__ == "__main__":
    bot_state["running"] = True
    threading.Thread(target=analysis_loop, daemon=True).start()
    add_log("XAUUSD KI-Bot v2.0 gestartet", "INFO")
